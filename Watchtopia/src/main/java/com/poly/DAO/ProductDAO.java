...
public interface ProductDAO extends JpaRepository<Products, Integer>{
	...
	@Query(value = "select c.product_id, p.product_img, p.product_name, p.product_price, p.types_id from cart c inner join product p on c.product_id = p.product_id"
			+ "where c.users_id = 1" , nativeQuery = true)
	List<Products> findCartByKeyWordBySQL();
}
